...
if ($result = $conn->query($sql)) {
    if($result->num_rows > 0 && $row = $result->fetch_assoc()){
        ...
        if ($conn->query($update)) {
            ...
            if($voulue) {
                $update = "DELETE FROM listBatiments WHERE id_village=".$id." AND id_batiment=1 AND x=".$x." AND y=".$y;
             }else {
                $update = "DELETE FROM listBatiments WHERE id_village=".$id." AND id_batiment=1  LIMIT ".$nbr;
             }
