...
if ($result = $conn->query($sql)) {
    if($result->num_rows > 0 && $row = $result->fetch_assoc()){
        ...
        if ($constrEnCours!=0){ //batiment deja en construction
            ...
            return FALSE;
        }else if($ress1<$ress1cas || $ress2<$ress2cas){ //pas suffisamment de ressources
            echo "Pas suffisamment de ressources pour construire le batiment : besoin de ". $ress1cas. " ress1 et ". $ress2cas. " ress2";
            require "disconnection.php";
            return FALSE;
        }
        /* On peut donc commencer la construction */
        $update = "UPDATE village SET ress1=". ($ress1-$ress1cas) .", ress2=". ($ress2-$ress2cas) .
            ", constrEnCours=2, constrDateDebut=\"".$last_maj. "\" WHERE id=" . $id;

        if ($conn->query($update)) {
           // echo "Lancement d'une construction d'une caserne du village ".$id. " <br />";
